...
const CLIENTS: Client[] = [
    ...
    employees: [
      {
        ...
        name: "Захаров Игорь",
        position: "CTO",
        email: "zakharov@gazprom.ru",
        phone: "+7 495 719-30-01",
        certified: true,
        certExpiry: "2027-04-15",
        docs: ["Договор_2025.pdf", "Акт_приёма.pdf"],
      },
      {
        id: 2,
        name: "Волкова Светлана",
        position: "IT-директор",
        email: "volkova@gazprom.ru",
        phone: "+7 495 719-30-02",
        certified: false,
        certExpiry: "",
        docs: [],
      },
    ...
    employees: [
      {
        ...
        position: "Директор",
        email: "morozov@tehnostroy.ru",
        phone: "+7 495 321-10-01",
        certified: true,
    ...
    employees: [
      {
        ...
        email: "popova@rostech.ru",
        phone: "+7 495 555-20-01",
        certified: true,
        certExpiry: "2025-11-20",
        docs: ["Контракт_Q1.pdf", "NDA.pdf", "Акт.pdf"],
      },
      {
        id: 5,
        name: "Громов Артём",
        position: "Менеджер проектов",
        email: "gromov@rostech.ru",
        phone: "+7 495 555-20-02",
        certified: false,
        certExpiry: "",
        docs: [],
      },
  ...
  },
  {
    ...
    type: "АО",
    inn: "7704567890",
    address: "Москва, ул. Пречистенка, 12",
    industry: "Нефтяная",
    status: "inactive",
    manager: "Новиков С.",
    contracts: 2,
  ...
  },
  {
    ...
    type: "ООО",
    inn: "7705678901",
    address: "Москва, ул. Садовническая, 8",
    industry: "Финансы",
    status: "lead",
    manager: "Петрова О.",
    contracts: 0,
  ...
  },
  {
    ...
    employees: [
      {
        id: 6,
        name: "Степанов Виктор",
        position: "Начальник IT",
        email: "stepanov@logistic.ru",
        phone: "+7 495 444-50-01",
        certified: true,
        certExpiry: "2027-12-01",
        docs: ["Договор.pdf"],
      },
    ],
  },
...
export default function ClientsPage() {
  ...
  const [filter, setFilter] = useState<"all" | "active" | "inactive" | "lead">(
    "all",
  );
  ...
  return (
    <div className="flex gap-4 h-full">
      ...
      <div
        ...
        style={{ width: selected ? "360px" : "100%", flexShrink: 0 }}
      >
        {/* Controls */}
        ...
        {/* Grid cards */}
        <div
          className={`grid gap-3 ${selected ? "grid-cols-1" : "grid-cols-3"}`}
        >
          {filtered.map((client) => (
            <div
              key={client.id}
      ...
      {selected && (
        <div
          className="flex-1 min-w-0 rounded-lg border border-border overflow-hidden animate-fade-in"
          ...
          <div className="flex border-b border-border">
            {(["info", "employees"] as const).map((tab) => (
              ...
              >
                {tab === "info"
                  ? "Информация"
                  : `Сотрудники (${selected.employees.length})`}
                {activeTab === tab && (
                  <div
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  { icon: "User", label: "Менеджер", value: selected.manager },
                  {
                    icon: "FileText",
                    label: "Контрактов",
                    value: selected.contracts.toString(),
                  },
                  {
                    icon: "TrendingUp",
                    label: "Выручка",
              ...
              <div className="space-y-3">
                {selected.employees.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <button
                      className="mt-3 text-xs text-white px-4 py-1.5 rounded hover:opacity-90"
                      style={{ background: "hsl(var(--primary))" }}
                    >
                      Добавить контакт
                    </button>
                      ...
                      >
                        <div className="flex items-start justify-between mb-3">
                          ...
                          {emp.certified && (
                            ...
                            >
                              <Icon
                                name={expired ? "AlertCircle" : "BadgeCheck"}
                                size={11}
                              />
                        ...
                        {emp.certExpiry && (
                          ...
                          >
                            ...
                            <span className="font-mono text-white">
                              {new Date(emp.certExpiry).toLocaleDateString(
                                "ru-RU",
                              )}
                            </span>
                          </div>
                        ...
                        {emp.docs.length > 0 && (
                          <div className="flex flex-wrap gap-1.5">
                            {emp.docs.map((doc) => (
                              <span
                                key={doc}
                                className="flex items-center gap-1 text-xs px-2 py-0.5 rounded cursor-pointer hover:opacity-80"
                                style={{
                ...
                <button
                  className="w-full py-2 border-2 border-dashed border-border rounded text-xs text-center hover:border-primary/50 transition-colors"
                  style={{ color: "hsl(var(--muted-foreground))" }}
                >
                  + Добавить контакт
                </button>
