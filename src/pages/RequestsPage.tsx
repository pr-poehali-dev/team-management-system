...
const MOCK_REQUESTS: Request[] = [
  { id: "REQ-2501", title: "Замена оборудования в серверной", description: "Требуется замена трёх серверов Dell R740", priority: "high", status: "inwork", assignee: "Иванов Д.", client: "ПАО Газпром", created: "01.03.2026", deadline: "10.03.2026" },
  { id: "REQ-2500", title: "Настройка VPN для удалённых сотрудников", description: "Настройка OpenVPN для 15 сотрудников", priority: "medium", status: "new", assignee: "Козлов П.", client: "ООО Технострой", created: "28.02.2026", deadline: "07.03.2026" },
  { id: "REQ-2499", title: "Обновление лицензий Microsoft Office", description: "Продление 38 лицензий Microsoft 365", priority: "low", status: "done", assignee: "Смирнова А.", client: "АО РосТех", created: "25.02.2026", deadline: "05.03.2026" },
  { id: "REQ-2498", title: "Аудит информационной безопасности", description: "Плановый аудит ИБ Q1 2026", priority: "high", status: "inwork", assignee: "Новиков С.", client: "ПАО Газпром", created: "24.02.2026", deadline: "15.03.2026" },
  { id: "REQ-2497", title: "Внедрение системы мониторинга", description: "Установка Zabbix для мониторинга 50 узлов", priority: "medium", status: "new", assignee: "—", client: "ООО Альфа", created: "22.02.2026", deadline: "20.03.2026" },
  { id: "REQ-2496", title: "Миграция данных на новый сервер", description: "Перенос 2TB данных с резервированием", priority: "high", status: "rejected", assignee: "Петрова О.", client: "АО Сибнефть", created: "20.02.2026", deadline: "01.03.2026" },
  { id: "REQ-2495", title: "Техническая поддержка склада №3", description: "Устранение проблем с сетевым оборудованием", priority: "medium", status: "done", assignee: "Иванов Д.", client: "ООО Логистик", created: "18.02.2026", deadline: "25.02.2026" },
];
...
export default function RequestsPage() {
  ...
  const [newReq, setNewReq] = useState({ title: "", description: "", priority: "medium" as Priority, client: "", deadline: "" });
            ...
            {filtered.map((req) => (
              <tr key={req.id} className="hover:bg-secondary/30 transition-colors group">
                <td className="px-4 py-3">
                  <span className="font-mono text-xs" style={{ color: "hsl(var(--primary))" }}>{req.id}</span>
                </td>
                ...
                <td className="px-4 py-3">
                  <span className="text-xs" style={{ color: "hsl(var(--foreground))" }}>{req.client}</span>
                </td>
                <td className="px-4 py-3">
                  <span className="text-xs px-2 py-0.5 rounded"
                    style={{ background: priorityConfig[req.priority].bg, color: priorityConfig[req.priority].color }}>
                    {priorityConfig[req.priority].label}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <span className="text-xs px-2 py-0.5 rounded"
                    style={{ background: statusConfig[req.status].bg, color: statusConfig[req.status].color }}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center"
          ...
          <div className="rounded-xl border border-border w-full max-w-lg p-6 space-y-4 animate-slide-up"
            ...
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setShowForm(false)} style={{ color: "hsl(var(--muted-foreground))" }}>
                <Icon name="X" size={18} />
              </button>
            </div>
            <div className="space-y-3">
              <div>
                <label className="text-xs mb-1 block" style={{ color: "hsl(var(--muted-foreground))" }}>Название</label>
                <input
                  value={newReq.title}
              ...
              <div>
                <label className="text-xs mb-1 block" style={{ color: "hsl(var(--muted-foreground))" }}>Описание</label>
                <textarea
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs mb-1 block" style={{ color: "hsl(var(--muted-foreground))" }}>Клиент</label>
                  <input
                    value={newReq.client}
                ...
                <div>
                  <label className="text-xs mb-1 block" style={{ color: "hsl(var(--muted-foreground))" }}>Приоритет</label>
                  <select
              ...
              <div>
                <label className="text-xs mb-1 block" style={{ color: "hsl(var(--muted-foreground))" }}>Дедлайн</label>
                <input
                  type="date"
